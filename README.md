...
I was planning to add graphics support but I abandoned the language before that could happen.
...
Quantities in Kid are unsigned integers that are dynamically sized (with a 1-byte minimum).
...
If `todos` was part of a space, that space will get loaded alongside any other contained blobs. Non-blobs won't load.
